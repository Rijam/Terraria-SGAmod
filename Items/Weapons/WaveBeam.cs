...
namespace SGAmod.Items.Weapons
{
	public class WaveBeam : SeriousSamWeapon,ITechItem, IDevItem
	{
		...
		{
			CreateRecipe(1).AddIngredient(mod.ItemType("NoviteBlaster"), 1).AddIngredient(ItemID.FragmentVortex, 6).AddIngredient(ItemID.FragmentNebula, 5).AddIngredient(mod.ItemType("AdvancedPlating"), 5).AddIngredient(mod.ItemType("PlasmaCell"), 2).AddIngredient(mod.ItemType("PrismalBar"), 6).AddIngredient(mod.ItemType("ManaBattery"), 2).AddIngredient(mod.ItemType("CosmicFragment"), 1).AddTile(mod.TileType("ReverseEngineeringStation")).Register();
		}
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("WaveBeamCharging").Type]<1){
			//if (altfired){
			int proj=Projectile.NewProjectile(position.X, position.Y, speedX, speedY, Mod.Find<ModProjectile>("WaveBeamCharging").Type, damage, knockBack, player.whoAmI);
	...
	public override bool? CanHitNPC(NPC target){return false;}
		...
		if (!channeling){
				...
				int proj=Projectile.NewProjectile(Projectile.Center.X, Projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, Mod.Find<ModProjectile>("ChargedWave").Type, Projectile.damage*15, Projectile.knockBack, player.whoAmI);
				...
				int proj=Projectile.NewProjectile(Projectile.Center.X, Projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, Mod.Find<ModProjectile>("WaveBeam").Type, Projectile.damage, Projectile.knockBack, player.whoAmI);
				Main.projectile[proj].timeLeft=120;
				Main.projectile[proj].penetrate=1;
				SoundEngine.PlaySound(Mod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/Wide_Beam_Shot").WithVolume(.7f).WithPitchVariance(.25f),Projectile.Center);
				}
