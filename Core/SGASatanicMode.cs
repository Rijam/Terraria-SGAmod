...
namespace SGAmod
{
    ...
    class SGASatanicGlobalItem : GlobalItem
    {
        ...
        public static bool DoCheatVisualEffect(Player player, bool cheatingOn = true)
        {
            if (!SGAmod.TotalCheating && !SGAmod.playCheatEffectOnce)
            {
                ...
                var snd = SoundEngine.PlaySound(SoundID.Item123, -1, -1);
                if (snd != null)
                {
                    snd.Pitch = -0.25f;
                }

                snd = SoundEngine.PlaySound(SoundID.Zombie, -1, -1, 96);
                if (snd != null)
                {
                    snd.Pitch = -0.5f;
                }
                ...
                if (explode != null)
                {
                    ...
                    explode.strengthBasedOnPercent = delegate (float percent)
                    {
                        ...
                        if (SGAWorld.modtimer % 30 == 0 && percent < 0.75f)
                        {
                            ...
                            var sound = SoundEngine.PlaySound(SGAmod.Instance.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/Heartbeat").WithPitchVariance(.025f), new Vector2(-1, -1));
                            if (sound != null)
                            {
                        ...
                        {
                            var sound = SoundEngine.PlaySound(SoundID.Zombie, -1, -1, 105);
                            if (sound != null)
                            {
        ...
        public override bool PreDrawTooltipLine(Item item, DrawableTooltipLine line, ref int yOffset)
        {
            ...
            {
                float alpha = MathHelper.Clamp((float)Math.Sin(Main.GlobalTimeWrappedHourly * 1.5f), 0f, 1f);
        ...
        public class ThereIsNoMercyThereIsNoInnocenceOnlyDegreesOfGuilt : Items.Consumables.TrueCopperWraithNotch
    {
        ...
        public static bool DrawnLine(DrawableTooltipLine line, ref int yOffset)
        {
            ...
            hallowed.Parameters["overlayTexture"].SetValue(ModContent.Request<Texture2D>("SGAmod/Fire"));
            ...
            hallowed.Parameters["overlayStrength"].SetValue(new Vector3(1f, 0.02f, Main.GlobalTimeWrappedHourly / 2f));
            ...
            hallowed.Parameters["overlayProgress"].SetValue(new Vector3(0, Main.GlobalTimeWrappedHourly / 11f, 0));
        ...
        {
            float scaleinandout = 3f + MathHelper.Clamp(1f + ((float)Math.Sin(Main.GlobalTimeWrappedHourly * 2f) * 2f), 0f, 3f) * 0.50f;
            ...
            {
                otherIndex += 1;
                spriteBatch.Draw(Main.itemTexture[Item.type], drawPos + (Vector2.UnitX.RotatedBy(((f) + Main.GlobalTimeWrappedHourly * 2f) * (otherIndex % 2 == 0 ? 1f : -1f)) * (scaleinandout * 1f)), null, Color.Black * 0.15f, 0, Main.itemTexture[Item.type].Size() / 2f, Main.inventoryScale / 2f, SpriteEffects.None, 0f);
            }
            ...
            {
                otherIndex += 1;
                spriteBatch.Draw(Main.itemTexture[Item.type], drawPos + (Vector2.UnitX.RotatedBy(((f) + Main.GlobalTimeWrappedHourly * 2f) * (otherIndex % 2 == 0 ? 1f : -1f)) * (scaleinandout + 0.5f)), null, Color.White * 0.30f, 0, Main.itemTexture[Item.type].Size() / 2f, Main.inventoryScale / 2f, SpriteEffects.None, 0f);
            }
