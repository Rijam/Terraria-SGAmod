...
{
    public class LaserSkeleton : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Laser Skeleton");
            Main.npcFrameCount[NPC.type] = Main.npcFrameCount[NPCID.TacticalSkeleton]; //21
        }
        ...
        public override void SetDefaults()
        {
            ...
            banner = NPC.type;
            bannerItem = Mod.Find<ModItem>("LaserSkeletonBanner").Type;
        }
        ...
        public override void NPCLoot()
        {
            ...
            //additionally, drop the Laser Rifle
            if (Main.rand.Next(20) == 0) //5% chance
            {
                Item.NewItem((int)NPC.position.X, (int)NPC.position.Y, NPC.width, NPC.height, ItemID.LaserRifle);
            }
        }
				...
				else
				{
					...
					if (NPC.ai[2] > 0f)
					{
						...
						if (NPC.ai[1] == num148)
						{
							...
							{
								projectileSpeedX = Main.player[NPC.target].position.X + (float)Main.player[NPC.target].width * 0.5f - vector31.X;
								projectileSpeedY = Main.player[NPC.target].position.Y + (float)Main.player[NPC.target].height * 0.5f - vector31.Y;
								num153 = (float)Math.Sqrt(projectileSpeedX * projectileSpeedX + projectileSpeedY * projectileSpeedY);
								num153 = 12f / num153;
								projectileSpeedX = (projectileSpeedX += (float)Main.rand.Next(-10, 11));
								projectileSpeedY = (projectileSpeedY += (float)Main.rand.Next(-10, 11));
								projectileSpeedX *= num153;
								projectileSpeedY *= num153;
								Projectile.NewProjectile(vector31.X, vector31.Y, projectileSpeedX, projectileSpeedY, projectileType, projectileDamage, 2.5f, Main.myPlayer);
							}
